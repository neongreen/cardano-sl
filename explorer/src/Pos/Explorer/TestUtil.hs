...
generateValidExplorerMockableMode blocksNumber slotsPerEpoch = do
    ...
    requiresNetworkMagic <- liftIO $ generate arbitrary

    blocks <- withDefConfigurations requiresNetworkMagic $ \_ _ _ ->
        produceBlocksByBlockNumberAndSlots blocksNumber slotsPerEpoch slotLeaders secretKeys

    let tipBlock         = Prelude.last blocks
    let pagedHHs         = withDefConfigurations requiresNetworkMagic $ \pm _ _ ->
                               createMapPageHHs blocks pm
    let hHsBlunds        = withDefConfigurations requiresNetworkMagic $ \pm _ _ ->
                               createMapHHsBlund blocks pm
    let epochPageHHs     = withDefConfigurations requiresNetworkMagic $ \pm _ _ ->
                               createMapEpochPageHHs blocks slotsPerEpoch pm
    let mapEpochMaxPages = withDefConfigurations requiresNetworkMagic $ \pm _ _ ->
                               createMapEpochMaxPages (keys epochPageHHs) pm
...
basicBlock prevHeader sk slotId = producePureBlock infLimit
                                                   prevHeader
                                                   []
                                                   Nothing
                                                   slotId
...
emptyBlk testableBlock =
    forAll arbitrary
        $ \(sk, prevHeader, slotId) -> testableBlock $ producePureBlock
              infLimit
              prevHeader
              []
              Nothing
