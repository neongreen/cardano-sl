...
main = do
    ...
    withCompileInfo $
        withConfigurationsM (LoggerName "verification-bench") Nothing cfo fn $ \ !coreConfig !txpConfig !_ ->
            let tp = TestParams
                ...
                secretKeys = maybe
                    (error "VerificationBench: No GeneratedSecrets available in config")
                    gsSecretKeys
                    (configGeneratedSecrets coreConfig)
            in runBlockTestMode tp secretKeys $ do
