...
spec = do
    ...
    describe "golden tests" $ do
        ...
        it "CBackupPhrase to JSON" $ forM_ testVectors $ \TestVector{..}  ->
            Aeson.encode (CBackupPhrase mnemonic) `shouldBe` jsonV0Compat bytes

        it "CBackupPhrase from JSON" $ forM_ testVectors $ \TestVector{..} ->
            Aeson.decode (jsonV0Compat bytes) `shouldBe` pure (CBackupPhrase mnemonic)

        it "CBackupPhrase from legacy JSON" $ forM_ testVectors $ \TestVector{..} ->
            Aeson.decode (jsonV0Compat legacyBytes) `shouldBe` pure (CBackupPhrase mnemonic)

        it "Mnemonic from legacy JSON" $ forM_ testVectors $ \TestVector{..} ->
  ...
  where
    ...
    testVectors =
        [ TestVector
            ...
            "[\"abandon abandon\",\"abandon\",\"abandon\",\"abandon abandon abandon\",\"abandon\",\"abandon\",\"abandon abandon\",\"about\"]"
            (orFail $ mkEntropy' "\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL\NUL")
            (orFail $ mkMnemonic
                ["abandon","abandon","abandon","abandon","abandon","abandon","abandon","abandon","abandon","abandon","abandon","about"])
